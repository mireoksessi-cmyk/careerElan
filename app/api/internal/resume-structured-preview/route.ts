...
/*
  TASK 9 - dev-only inspection endpoint for the Resume Structured Model
  (lib/documentPreservation/resumeStructured/). Mirrors
  app/api/internal/resume-semantic-preview/route.ts's own established
  convention exactly (same fixture-path safety check, same Netlify-
  runtime gate, same unauthenticated-but-local-only posture) - the only
  difference is this route also runs Phase 2's buildStructuredResume on
  top of Phase 1's own output and returns both, so the inspection UI can
  show them side by side (spec section 20).
*/
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unknown error while running the structured resume engine." },
      { status: 500 }
    );
  }
}
