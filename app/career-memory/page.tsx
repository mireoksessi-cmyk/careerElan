...
export default function CareerMemoryPage() {
  ...
  const strength = mode === "start" ? 0 : memoryStrength();
  const requiredCount = requiredCompletedCount();

  function persistMemory() {
    localStorage.setItem("careerMemoryData", JSON.stringify(memoryData));
    localStorage.setItem("careerMemoryRequiredCompleted", String(canUseService()));
    localStorage.setItem("careerMemoryStrength", String(memoryStrength()));
  }
  ...
  function continueToDashboard() {
    if (!canUseService()) {
      setLockedMessage("Complete the 3 required sections first: Personal Information, Experience, and Skills.");
      return;
    }
    persistMemory();
    ...
    return (
      <div className="mt-8 rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          ...
          <Select label="Cover Letter Template" value={memoryData.coverLetterTemplate} onChange={(v) => updateMemory("coverLetterTemplate", v)} items={coverLetterTemplates} />
    ...
    return (
      <div className="mx-auto min-h-[960px] w-full max-w-[760px] bg-white p-8 shadow-xl sm:p-10">
        ...
        <ResumeSection title="Experience">
          {memoryData.workExperience.filter((x) => x.company || x.jobTitle || x.description).map((x, i) => (
            <div key={`work-${i}`} className="mb-5">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-5"><li>{x.description || "Experience details will appear here."}</li></ul>
            </div>
          ))}
        ...
        <ResumeSection title="Education">
          {memoryData.education.filter((x) => x.school || x.program).map((x, i) => (
            <div key={i} className="mb-3">
              <div className="flex flex-col gap-1 sm:flex-row sm:justify-between sm:gap-4"><p className="font-black text-slate-950">{x.program || "Program / Degree"}</p><p className="text-sm text-slate-500">{x.dates || "Dates"}</p></div>
              <p className="font-bold text-slate-700">{x.school || "School Name"}</p>
              {x.coursework && <p className="mt-1 text-slate-600">{x.coursework}</p>}
            </div>
          ))}
        </ResumeSection>
        <ResumeSection title="Skills"><p>{memoryData.skills || "Your skills will appear here."}</p></ResumeSection>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-3"><button onClick={() => (mode === "import" ? setImportStage("parsed") : setCurrentStep(7))} className="rounded-xl border border-blue-600 px-5 py-3 font-bold text-blue-600">Back</button><button onClick={continueToDashboard} disabled={!canUseService()} className={`rounded-xl px-5 py-3 font-bold ${canUseService() ? "bg-blue-600 text-white" : "cursor-not-allowed bg-slate-100 text-slate-400"}`}>Save & Continue</button></div>
        </div>
    ...
    return (
      <div className="mb-6 rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <button type="button" onClick={continueToDashboard} disabled={!canUseService()} className={`rounded-xl px-6 py-3 text-sm font-black transition ${canUseService() ? "bg-blue-600 text-white shadow-lg shadow-blue-100 hover:bg-blue-700" : "cursor-not-allowed bg-slate-100 text-slate-400"}`}>Continue to Dashboard →</button>
        </div>
    ...
    <main className="min-h-screen bg-[#f6fbff] text-gray-900">
      <div className="flex min-h-screen">
        ...
        <section className="flex-1 px-8 py-6">
          {mode === "start" ? (
            <StartScreen strength={strength} requiredCount={requiredCount} canUseService={canUseService()} onImport={() => setMode("import")} onBuild={() => setMode("build")} onContinue={continueToDashboard} />
          ) : (
            <>
...
function StartScreen({ strength, requiredCount, canUseService, onImport, onBuild, onContinue }: { strength: number; requiredCount: number; canUseService: boolean; onImport: () => void; onBuild: () => void; onContinue: () => void }) {
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr_310px]"><button onClick={onImport} className="rounded-3xl border border-blue-100 bg-white p-8 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-green-50 text-4xl">☁️</div><h2 className="mt-7 text-2xl font-black text-slate-950">Import Your Resume</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-500">Upload your current resume and let AI extract your information automatically.</p><div className="mt-6 grid gap-3 text-sm font-semibold text-slate-600 sm:grid-cols-2">{["Work Experience", "Skills & Competencies", "Education", "Projects", "Certifications", "Achievements"].map((x) => <p key={x}><span className="text-green-600">✓</span> {x}</p>)}</div><div className="mt-8 rounded-xl bg-blue-600 px-6 py-4 text-center font-black text-white">Upload Resume</div><p className="mt-4 text-center text-xs font-semibold text-slate-400">🔒 Your data is secure and private</p></button><button onClick={onBuild} className="rounded-3xl border border-blue-100 bg-white p-8 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-50 text-4xl">✎</div><h2 className="mt-7 text-2xl font-black text-slate-950">No Resume?</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-500">Answer a few simple questions and build your Career Memory step-by-step.</p><div className="mt-6 space-y-3 text-sm font-semibold text-slate-600"><p>✓ Required sections first</p><p>✓ Optional sections later</p><p>▣ Save and continue anytime</p></div><div className="mt-9 rounded-xl border border-blue-600 px-6 py-4 text-center font-black text-blue-600">Start Building</div><p className="mt-4 text-center text-xs font-semibold text-slate-400">🔒 Your data is secure and private</p></button><aside className="space-y-5"><div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm"><h3 className="text-center text-base font-black text-slate-950">Career Memory Status</h3><div className="mx-auto mt-6 flex h-36 w-36 items-center justify-center rounded-full border-[12px] border-slate-100"><div className="text-center"><p className="text-3xl font-black text-slate-950">{strength}%</p><p className="text-xs font-semibold leading-5 text-slate-500">Overall Profile<br />Strength</p></div></div><div className="mt-6 space-y-4 text-sm font-semibold text-slate-500"><div className="flex items-center justify-between"><span>👤 Personal Info</span><span className="text-slate-400">○</span></div><div className="flex items-center justify-between"><span>💼 Experience</span><span className="text-slate-400">○</span></div><div className="flex items-center justify-between"><span>🔐 Skills</span><span className="text-slate-400">○</span></div><div className="flex items-center justify-between"><span>🎓 Education</span><span className="text-slate-400">○</span></div><div className="flex items-center justify-between"><span>⌁ Projects</span><span className="text-slate-400">○</span></div></div><div className="mt-5 border-t border-slate-100 pt-5 text-center text-xs font-semibold text-slate-400">Complete 3 required sections to unlock the app.</div></div><div className="rounded-3xl border border-blue-100 bg-blue-50 p-6 shadow-sm"><h3 className="font-black text-blue-700">🛡️ Your data is 100% secure</h3><p className="mt-4 text-sm leading-6 text-slate-600">We use secure authentication and never sell or share your personal information.</p><p className="mt-4 text-sm font-black text-blue-600">Learn more →</p></div></aside></div>
