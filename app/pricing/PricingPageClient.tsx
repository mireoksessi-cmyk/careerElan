...
/*
  Phase 6I.6.28 - Log in/Get Started/Start (Header) and the Free plan's
  Get Started now open the real Homepage auth modal directly (via the
  shared PublicAuthActionsProvider), instead of linking back to "/". The
  Pro "Coming Soon" button stays a disabled, non-interactive button -
  untouched by this phase (no Stripe/Checkout/Subscription wiring).
  Metadata stays in app/pricing/page.tsx (a Server Component) since a
  "use client" file cannot export `metadata` - this file is that page's
  client-rendered body.

  Content-truth notes (carried over from Phase 6I.6.26):
  - "3 Generate Packages / month" mirrors the ACTUAL current policy
    ...
    monthly_generation_limit = 3, UTC calendar month).
  - The free-plan feature list reproduces ONLY items already verified as
    real, live product capabilities in Phase 6I.6.24/25's own audits.
  - Pro is intentionally priceless and quota-less here: quota_plans.pro's
    internal DB value (50/month) is NOT surfaced to users.
*/
