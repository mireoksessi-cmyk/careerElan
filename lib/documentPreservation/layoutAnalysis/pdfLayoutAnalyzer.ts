...
/*
  Production-safe PDF.js loading. pdfjs-dist@6.1.200's legacy Node build
  (legacy/build/pdf.mjs) references the bare global `DOMMatrix` at its own
  module top level (`const SCALE_MATRIX = new DOMMatrix();` inside its
  bundled display/canvas.js section) - this runs unconditionally on every
  import, before any of our own code (getDocument/getPage/getTextContent)
  ever executes, regardless of whether the caller renders anything. On
  Netlify Production this throws `ReferenceError: DOMMatrix is not
  defined`, confirmed via captured Function logs: pdfjs-dist's own
  Node-init block (isNodeJS branch) tries `require("@napi-rs/canvas")` to
  source a DOMMatrix polyfill, that require fails ("Cannot load
  "@napi-rs/canvas" package"), so it just warns ("Cannot polyfill
  DOMMatrix") and leaves globalThis.DOMMatrix unset - the later bare
  `new DOMMatrix()` then throws for real, and this analyzer never even
  reaches its own getDocument() call.

  This analyzer never calls page.render() (only getTextContent/
  getViewport/getOperatorList, confirmed - no canvas pixel output is ever
  produced or needed here), so a full canvas-rendering DOMMatrix
  implementation is unnecessary. An exhaustive grep of the installed
  pdfjs-dist@6.1.200 bundle for every `new DOMMatrix(`/`.multiplySelf(`/
  `.preMultiplySelf(`/`.invertSelf(` occurrence confirms every one of them,
  other than the single top-level `SCALE_MATRIX` construction above, is
  reached only via a real canvas 2D context's own `getTransform()`/Path2D
  APIs - i.e. exclusively inside page.render()'s own code paths, which
  this analyzer never invokes. The only genuinely required surface is
  therefore: a no-arg/6-value-array constructor, the a/b/c/d/e/f
  properties, and multiplySelf/preMultiplySelf/invertSelf/translate/scale
  - implemented below with real 2D affine matrix math (the same
  composition convention as this file's own multiplyMatrix() below, which
  matches pdfjs-dist's own internal Util.transform()), not identity
  no-ops, so nothing downstream (geometry, section detection, lossless
  validation) is silently corrupted if any of these methods ever is
  reached. Never installed when a real DOMMatrix already exists (local
  dev, or any future runtime that supplies one natively) - that behavior
  is untouched.
*/
class MinimalServerDOMMatrix {
  ...
  // Same composition convention as this file's own multiplyMatrix() and
  // pdfjs-dist's own Util.transform(): result = m1 (applied second)
  // composed with m2 (applied first) - i.e. m1 "post-multiplied" by m2.
