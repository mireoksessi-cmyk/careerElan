...
function splitSkillList(text: string): string[] {
  ...
  if (!delimiter) return [text.trim()].filter((s) => s.length > 0);
  return text
    .split(delimiter)
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
...
export function extractSkillGroups(sectionId: string, bodyBlocks: SemanticContentBlock[]): SkillGroup[] {
  ...
  for (const block of relevant) {
    ...
    // A prose/sentence block contributes no skill VALUE (per the
    // "never split a sentence into skills" rule above) but its block id
    // must still be traced somewhere - real fixture evidence
    // (bench/resume-E-senior-ats.pdf) showed a "Skills"-typed section
    // whose tail also contains prose bullet fragments (Phase 1's own
    // section boundary, not corrected this round); dropping those
    // blocks from coverage entirely violated the "never silently drop a
    // block" invariant even though correctly extracting zero skills
    // from them.
    ungroupedBlocks.push(block);
    if (!isSentence(text)) {
      ungroupedSkills.push(...splitSkillList(text));
    }
  }
