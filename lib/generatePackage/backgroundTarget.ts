...
/*
  ...
  origin-resolution logic (currently: analyze-resume-background,
  analyze-cover-letter-background - see lib/documentAnalysis/). Deliberately
  a separate function rather than a refactor of resolveBackgroundFunctionUrl
  itself: that function's own call site (app/api/generate-package/route.ts)
  is already verified in Production, and this module's whole purpose is
  precisely to avoid ever risking that path - see this file's own top
  docstring on why even its zero-dependency design exists to be safe to
  import from more places, not to be edited in place.
*/
